...
public class ExcelUtil {
        ...
        while (it.hasNext()) {
            ...
            try {
                if (t instanceof Map) {
                    ...
                    List<FieldForSorting> fields = sortFieldByAnnotation(t.getClass());
                    int cellNum = 0;
     ...
     */
    public static <T> Collection<T> importExcel(Class<T> clazz, InputStream inputStream, ExcelLogs logs) {
        return importExcel(clazz, inputStream, DATE_PATTERN_DEFAULT_LONG, logs, true, 0);
    }
    ...
    public static <T> Collection<T> importExcel(Class<T> clazz, InputStream inputStream,
                                                String pattern, ExcelLogs logs, boolean byIndex, Integer... arrayCount) {
        Workbook workBook;
        try {
        ...
        try {
            ...
            while (rowIterator.hasNext()) {
                ...
                if (clazz == Map.class) {
                    ...
                    for (Object k : titleMap.keySet()) {
                        Integer index = null;
                        if (byIndex) {
                            index = (Integer) k;
                        } else {
                            index = (Integer) titleMap.get(k);
                        }
                    ...
                    List<FieldForSorting> fields = sortFieldByAnnotation(clazz);
    ...
    private static String validateCell(Cell cell, Field field, int cellNum) {
        ...
        ExcelCell annotationCell = field.getAnnotation(ExcelCell.class);
        if (cell == null
                || (cell.getCellTypeEnum() == CellType.STRING && StringUtils.isBlank(cell
                .getStringCellValue()))) {
            if (annotationCell != null && annotationCell.valid().allowNull() == false) {
                result = MessageFormat.format("the cell [{0}] can not null", columnName);
            }
            ;
        } else if (cell.getCellTypeEnum() == CellType.BLANK && annotationCell.valid().allowNull()) {
            return result;
        } else {
            ...
            if (!(cellTypes.contains(cell.getCellTypeEnum()))
                    || StringUtils.isNotBlank(annotationCell.defaultValue())
                    && cell.getCellTypeEnum() == CellType.STRING) {
                ...
                // 类型符合验证,但值不在要求范围内的
                // String in
                if (annotationCell.valid().in().length != 0 && cell.getCellTypeEnum() == CellType.STRING) {
                    String[] in = annotationCell.valid().in();
                ...
                if (cell.getCellTypeEnum() == CellType.NUMERIC) {
                    ...
                    // 小于
                    if (!Double.isNaN(annotationCell.valid().lt())) {
                        if (!(cellValue < annotationCell.valid().lt())) {
                            result =
                                    MessageFormat.format("the cell [{0}] value must less than [{1}]", columnName,
                                            annotationCell.valid().lt());
                        }
                    }
                    // 大于
                    if (!Double.isNaN(annotationCell.valid().gt())) {
                        if (!(cellValue > annotationCell.valid().gt())) {
                            result =
                                    MessageFormat.format("the cell [{0}] value must greater than [{1}]", columnName,
                                            annotationCell.valid().gt());
                        }
                    }
                    // 小于等于
                    if (!Double.isNaN(annotationCell.valid().le())) {
                        if (!(cellValue <= annotationCell.valid().le())) {
                            result =
                                    MessageFormat.format("the cell [{0}] value must less than or equal [{1}]",
                                            columnName, annotationCell.valid().le());
                        }
                    }
                    // 大于等于
                    if (!Double.isNaN(annotationCell.valid().ge())) {
                        if (!(cellValue >= annotationCell.valid().ge())) {
                            result =
                                    MessageFormat.format("the cell [{0}] value must greater than or equal [{1}]",
                                            columnName, annotationCell.valid().ge());
                        }
                    }
                }
            }
     ...
     * @return
     */
    private static List<FieldForSorting> sortFieldByAnnotation(Class<?> clazz) {
        Field[] fieldsArr = clazz.getDeclaredFields();
        List<FieldForSorting> fields = new ArrayList<>();
        List<FieldForSorting> annotationNullFields = new ArrayList<>();
        ...
        fields.addAll(annotationNullFields);
